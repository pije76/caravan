...
def getTotalUsers(except_email, filter_str):
    user_list = list(tbl_user.objects.filter().exclude(username=except_email))
    res = []
...
def registerUser(request):
    try:
        ...
        # ---------- duplication check ---------- #
        user_obj = list(tbl_user.objects.filter(email=data["email"]))
        if len(user_obj) > 0:
            return HttpResponse("existing_email", stauts=500)
        user_obj = list(tbl_user.objects.filter(consumer_id=data["consumer_id"]))
        if len(user_obj) > 0:
...
#        user_obj.uid = 0
        ...
        try:
            ...
            for iccid in iccid_list:
                iccid_obj = list(tbl_iccid_list.objects.filter(iccid=iccid))
                if len(iccid_obj) != 0:
                    continue
                iccid_obj = tbl_iccid_list()
...
def updateUser(request):
    try:
        ...
        # ---------- duplication check ---------- #
        user_obj = list(tbl_user.objects.filter(consumer_id=data["consumer_id"]).exclude(email=data["email"]))
        if len(user_obj) > 0:
            return HttpResponse("existing_consumer_id", status=500)
